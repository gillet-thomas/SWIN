...
def plot_embeddings(model, dataloader, config, mode):
    ...
    plt.title(f'{method.upper()} of 4-targets Embeddings')
...
def get_file_name(method, mode, param=None):
    if param is None:
        return f'{method}_4targets_{mode}_{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}.png'
    else:
        return f'{method}_4targets_{mode}_{param}.png' # param is perplexity or neighbors


# Load model
cuda_id = 2
config = yaml.safe_load(open("data/config.yaml", "r"))
best_model_path = config[f'best_swin_4targets']
