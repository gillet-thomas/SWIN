...
import yaml
from custom_training import ADNISwiFTDataset, Model

warnings.simplefilter(action='ignore', category=FutureWarning)

cuda_id = 3
target = 1
task = 'age_group' # age_group or sex
generate_maps = False
max_iter = 20
save_dir = os.path.join(os.getcwd(),'visualization')
pretrained_ckpt_path = f'/mnt/data/iai/Projects/ABCDE/fmris/CLIP_fmris/SwiFT/results/2025-06-26_13-18-58/model-e29.pth'
    ...
    for idx, (subject, _, _, start_frame_idx) in enumerate(tqdm(test_loader),0):
        ...
        if pred_int == data_target: # Of the subjects that corrected
            if (data_target == 0 and pred_prob <= 0.25) or (data_target == 1 and pred_prob >= 0.75):
                file_dir = os.path.join(save_dir, f'ADNI_{config["downstream_task"]}_target{data_target}')
...
target_affine = nb.load('/mnt/data/iai/datasets/ADNI_CONN_conversion/corresponding_processed/136_S_4993_I342514/wauI342514_Resting_State_fMRI_136_S_4993.nii').affine
...
IG_dir = f'{save_dir}/ADNI_{config["downstream_task"]}_target{target}/*'
